...
void analiseBuscaS1(int N,double mediaTempo, double inicio, double fim, Estatisticas* statistics, Hash* hash, vector<Data_Casos> casos){
...
bool analise(){
    ...
    for(int i = 0; i< M; i++){
        for(int j = 0; j < M; j++){
            analiseBuscaS1(N[i], mediaTempo, inicio, fim, statistics, hash, casos);
        }
        statistics->clear();
    }
return true;    
